...
auto ChfsClient::readdir(inode_id_t id)
  ...
  auto pair = response.unwrap()->as<std::vector<std::pair<std::string, inode_id_t>>>();
  return ChfsResult<std::vector<std::pair<std::string, inode_id_t>>>(pair);
}
...
auto ChfsClient::get_type_attr(inode_id_t id)
  ...
  auto result = response.unwrap()->as<std::tuple<u64, u64, u64, u64, u8>>();
  auto [size, atime, mtime, ctime, type] = result;
  InodeType inodeType;
...
auto ChfsClient::read_file(inode_id_t id, usize offset, usize size)
  ...
  std::vector<u8> result = std::vector<u8>(size);
...
auto ChfsClient::write_file(inode_id_t id, usize offset, std::vector<u8> data)
  ...
  {
    auto [blockId, macId, versionId] = *it;
    if (data_servers_.find(macId) == data_servers_.end()){

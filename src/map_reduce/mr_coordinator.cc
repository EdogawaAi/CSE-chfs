...
namespace mapReduce {
    std::tuple<int, int, std::string, int, int> Coordinator::askTask(int) {
        ...
        if (!isMapFinished)
        {
            int index = -1;
            int i = 0;
            for (auto iter = map_task_handout.begin(); iter != map_task_handout.end(); iter++, i++)
            {
                if (*iter == false)
                {
                    index = i;
        ...
        else if (!isReduceFinished)
        {
            int index = -1;
            int i = 0;
            for (auto iter = reduce_task_handout.begin(); iter != reduce_task_handout.end(); iter++, i++)
            {
                if (*iter == false)
                {
                    index = i;
    ...
    int Coordinator::submitTask(int taskType, int index) {
        ...
        if (taskType == mapReduce::mr_tasktype::MAP)
        {
            ...
            for (auto iter = map_task_finish.begin(); iter != map_task_finish.end(); iter++)
            {
                if (*iter == false)
                {
        ...
        else if (taskType == mapReduce::mr_tasktype::REDUCE)
        {
            ...
            for (auto iter = reduce_task_finish.begin(); iter != reduce_task_finish.end(); iter++)
            {
                if (*iter == false)
                {
